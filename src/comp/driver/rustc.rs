...
fn compile_input(sess: session::session, cfg: ast::crate_cfg, input: str,
    ...
    if sess.building_library() {
        // the library was just built with a name like libfoo.so
        // as demand by make, rename it to include hash and version
        // in filename
        link::rename_library(outputs.out_filename, link_meta);
    }
}
